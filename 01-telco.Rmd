...
Dari hasil yang diperoleh pada `model_forest`, nilai Out of Bag Error (OOB) sebesar 21.61%. Artinya accuracy pada model tersebut sebesar 78.39%. Error yang diperoleh masih cukup tinggi. Selanjutnya, akan dilakukan tuning model dengan melakukan upsample data. Artinya, kita akan membuat proporsi dari target variabel sama besar.
```{r}
...
```{r}
forest_upc$finalModel
```

Setelah dilakukan upsample data, terlihat nilai OOB yang diperoleh lebih kecil dibandingkan model sebelumnya yaitu sebesar 9.03%. Selanjutnya, akan dilakukan prediksi terhadap `data_test`:
